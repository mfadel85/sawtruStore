        ...
        <div class="qc-setting-tab-content"> 
            <div class="qc-setting-tab-pane fade in active" role="tabpanel" id="{ opts.setting_id }_general" aria-labelledby="{ opts.setting_id }_general_tab"> 
                <div class="ve-editor__setting__content__section">
                    ...
                    <div class="ve-field">
                        <label class="ve-label">{getLanguage().general.text_display} search</label>
                        <div>
                            <qc_switcher 
                                onclick="{parent.edit}" 
                                name="config[guest][{opts.step}][fields][{ opts.field_id }][search]" 
                                data-label-text="Enabled" 
                                value={ getState().config.guest[opts.step].fields[opts.field_id].search } 
                            />
                        </div>
                    </div>
                    ...
                    <div class="ve-field">
                        ...
                        <select
                            name="config[{getAccount()}][{opts.step}][fields][{ opts.field_id }][value]"
                            class="ve-input"
                            no-reorder
                            onchange="{parent.edit}" >
                            <option value="" selected={ isEmpty(opts.field.value)} >{ getLanguage()[opts.step][opts.field.placeholder] }</option>
                            <option
                                each={option in opts.field.options }
                                if={option}
                                value={ option.value }
                                selected={ option.value == opts.field.value} >
                                { option.name } 
                            </option>
                        </select>
                    </div>
